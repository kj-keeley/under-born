...
ゲームは既に遊べる状態にある（`game/underborn_scout8a.html`）。
**2026-09-03 に中断を解除**（DECISIONS 運用・体制）。遺跡16階便まで実装済み。以後は便を Kevin 待ち無しで連続実行し、Kevin は最終決定4つだけを持つ。
...
- [ ] 遺跡16階の通し結果を見て、他4土地を16階型へ揃えるか、土地ごとに階数・区画数を変えるか決める
...
- 一般配布用の `DEV_DEFAULT=false` 戻し、遺跡16階の長い通しプレイ、他4土地の16階化、バランス数値の確定
