...
ゲームは既に遊べる状態にある（`game/underborn_scout7bg.html`）。
...
- [ ] **Codex（Sol）**：`docs/orders/2026-09-06_7B-b_v2.md` を実装 → `game/underborn_scout7bh.html`（測定に依らない4項目：クリア保存の前倒し／地上演出と土地の詩／タイトルの卵と孵化／地図トグル廃止とミニマップ、＋森コア＝息吹）。コード検索で裏を取る
